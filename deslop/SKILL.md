...
5. **Launch all 8 agents in parallel** — single message, 8 `Agent` tool calls with `subagent_type: "general-purpose"`. Each agent gets:
...
6. **Consolidate findings** — merge the 8 reports. Deduplicate overlap (agents 1 and 2 often find the same shared-type issue). Group by file. Sort by severity. Write the full consolidated report to `deslop-report.md` at the repo root.
...
8. **Apply changes (branch + worktree fan-out)** — when user approves:
   ...
   3. After each worktree returns, merge it back to the cleanup branch in the precedence order: deletion (3, 7) → simplification (6, 8) → consolidation (1, 2) → typing (5) → structural (4). Run build + typecheck + tests between each merge. Stop and surface on regressions.
